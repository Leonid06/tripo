...
from celery_microservice.config import RABBITMQ_TEST_EXCHANGE_NAME, RABBITMQ_INWARD_TOPIC_NAME, \
    RABBITMQ_OUTWARD_TOPIC_NAME


@app.task
def consume_test_topic_message_task():
    inward_consumer = TopicConsumer()
    inward_consumer.topic = RABBITMQ_INWARD_TOPIC_NAME
    inward_consumer.exchange = RABBITMQ_TEST_EXCHANGE_NAME
...
def produce_test_topic_message_task(message_id: str, query: str, latitude: str, longitude: str, radius: str):
    ...
    outward_producer.topic = RABBITMQ_OUTWARD_TOPIC_NAME
    outward_producer.exchange = RABBITMQ_TEST_EXCHANGE_NAME
